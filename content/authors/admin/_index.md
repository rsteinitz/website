---
# Display name
title: Ronnie Bailey-Steinitz

# Name pronunciation (optional)
name_pronunciation: ""

# Full name (for SEO)
first_name: Ronnie
last_name: Bailey-Steinitz

superuser: true
highlight_name: true

role: Wildlife Biologist | Data Scientist

organizations:
  - name: "Dian Fossey Gorilla Fund"
    url: "https://gorillafund.org/"
  - name: "San Diego, CA"
    url: ""

# ======================
# SOCIAL PROFILES
# ======================
profiles:
  - icon: hero/at-symbol
    url: "mailto:ronnie.steinitz@gmail.com"
    label: "Email Me"

  - icon: brands/github
    url: "https://github.com/rsteinitz"
    label: "GitHub"

  - icon: brands/linkedin
    url: "https://www.linkedin.com/in/ronniesteinitz/"
    label: "LinkedIn"

  - icon: academicons/google-scholar
    url: "https://scholar.google.com/citations?hl=en&authuser=2&user=astDl-wAAAAJ"
    label: "Google Scholar"

  - icon: academicons/orcid
    url: "https://orcid.org/0000-0001-8719-9080"
    label: "ORCiD"

  - icon: brands/instagram
    url: "https://www.instagram.com/RoamingEcologist"
    label: "Instagram"

# ======================
# INTERESTS
# ======================
interests:
  - Wildlife Biology
  - Physiology and Energetics
  - Food Web Dynamics
  - Spatial Ecology
  - R, GIS

# ======================
# EDUCATION
# ======================
education:
  - area: Ph.D. Integrative Anthropological Sciences
    institution: University of California, Santa Barbara
    date_start: 2017-09-15
    date_end: 2024-09-14
    summary: |
      Dissertation Title: _The Tall Price of Being Small: The Energetic Consequences of Interspecific Feeding Competition in Primates_  
      Supervisor: Drs. Michael Gurven and Michelle Brown  
      - $89,394 in fellowships and $48,462 in research grants  
      - 1 first-author publication and 5 first-author conferences presentations  
      - 2 manuscripts in preparation  
      - 5 invited talks  
      - 1 Academic award, 4 nominations for Excellence in Teaching awards  
      - Supervised 13 undergraduate and post-grad students

    button:
      text: "Read Dissertation"
      url: "https://www.proquest.com/docview/3123627169/BBE78CF9180B4C33PQ/1"

  - area: M.A. Integrative Anthropological Sciences
    institution: University of California, Santa Barbara
    date_start: 2017-09-15
    date_end: 2021-04-14
    summary: |
      Thesis Title: _Energy consequences of intra-guild feeding competition in a primate community._
      Supervisor: Dr. Michelle Brown
      - International permits research and sample export
      - Designed and carried out pilot season for PhD dissertation

  - area: M.S. Biology
    institution: University of California, San Diego
    date_start: 2014-01-01
    date_end: 2015-06-15
    summary: |
      Thesis Title: _Stable isotope discrimination factors for terrestrial reptiles._  
      Supervisor: Dr. Carolyn Kurle
      - $3,500 in fellowships and $3,000 in research grants  
      - 2 first-author publications and 2 first-author conferences presentations  
      - Excellence in Teaching award

  - area: B.S. Ecology, Behavior, and Evolution
    institution: University of California, San Diego
    date_start: 2009-09-15
    date_end: 2013-12-31

# ======================
# WORK EXPERIENCE
# ======================
work:
  - position: Postdoctoral Scientist
    company_name: Dian Fossey Gorilla Fund
    date_start: 2025-03-01
    summary: |
      - Research health and aging in wild mountain gorillas  
      - Develop health trajectory models using behavioral and physiological data  
      - Collaborate with international partners and mentor Rwandan students  
      - Coordinate research projects at the Ellen DeGeneres Campus in Rwanda  

  - position: Lecturer - Primate Behavior & Human-Wildlife Conflict
    company_name: University of California, Santa Barbara
    date_start: 2024-09-20
    date_end: 2024-12-31
    summary: |
      - Created interdisciplinary, biodiversity-focused curricula  
      - Taught ~50 students per class  
      - Improved course performance via data-informed teaching  
      - Mentored undergraduates in professional development

  - position: Ph.D. Candidate
    company_name: University of California, Santa Barbara
    date_start: 2017-09-15
    date_end: 2024-09-14
    summary: |
      - Led multi-phase primate research project  
      - Developed metabolic biomarker panel  
      - Analyzed biological and spatial data (R, GIS)
      - Secured grants, permits, and coordinated international fieldwork  
      - Publications and conferences presentations
      - Mentored 13 undergraduates

  - position: Researcher
    company_name: U.S. Geological Survey
    date_start: 2015-09-15
    date_end: 2017-09-14
    summary: |
      - Designed bobcat feeding ecology study  
      - Analyzed stable isotopes and blood biomarkers  
      - Managed research funding and logistics

# ======================
# SKILLS (fixed icons)
# ======================
skills:
  - name: Technical Skills
    items:
      - name: Data Science
        icon: chart-bar
        icon_pack: fas
        percent: 100

      - name: R Programming
        icon: code-bracket
        icon_pack: fas
        percent: 100

      - name: GIS
        icon: "custom/earth-grid"
        icon_pack: custom
        percent: 100

      - name: SQL
        icon: circle-stack
        icon_pack: fas
        percent: 80

  - name: Hobbies
    color: "#eeac02"
    color_border: "#f0bf23"
    items:
      - name: Photography
        icon: camera
        icon_pack: fas
        percent: 80

      - name: Backpacking
        icon_pack: hero
        icon: map
        percent: 60

      - name: Graphic design
        icon: "custom/wand-magic-white"
        icon_pack: custom
        percent: 100

  - name: Search & Rescue
    color: "#eeac02"
    color_border: "#f0bf23"
    items:
      - name: Rope Rescue
        icon: "custom/figure8knot2"
        icon_pack: custom
        percent: 100

      - name: Navigation
        icon: "custom/compass-white"
        icon_pack: custom
        percent: 80

      - name: Medical Response
        icon: "custom/sar"
        icon_pack: custom
        percent: 60

# ======================
# AWARDS 
# ======================
awards:

  - title: NSF Doctoral Dissertation Research Improvement Grant (DDRIG)
    awarder: National Science Foundation
    date: "2022-08-01"
    icon: trophy
    icon_pack: fas
    summary: |
      ($31,062) Supported multi-site fieldwork on primate energetics and competition.

  - title: UC Graduate Opportunity Fellowship
    awarder: University of California
    date: "2021-04-01"
    icon: trophy
    icon_pack: fas
    summary: |
      ($42,435) Funded doctoral research on primate energy balance and competition.

  - title: USGS Independent Research Grant
    awarder: USGS
    date: "2017-08-01"
    icon: trophy
    icon_pack: fas
    summary: |
      Supported bobcat feeding ecology research during California Drought.

  - title: Excellence in Teaching Award
    awarder: UC San Diego
    date: "2014-09-01"
    icon: graduation-cap
    icon_pack: fas
    summary: |
      Recognized for outstanding undergraduate instruction and mentorship.

---

## About Me

I am a wildlife biologist and data scientist. My research explores feeding ecology and food-web dynamics using advanced analysis and modeling in R and GIS. I study primate physiology, energetics, and aging, bobcat foraging, and ecological niche partitioning in endangered species.

I welcome collaborations in conservation research and data science.
